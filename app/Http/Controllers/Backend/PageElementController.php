...
class PageElementController extends Controller
{
    ...
    public function store(Request $request){
        $request->validate([
            'bg_image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:1048',
        ...
        if(PageElement::where('type','=', 1)->first()){
            ...
            if($request->file('bg_image')){
                    $singlefiles = $request->file('bg_image');
                    
                    $customnamefile='bg_image'.rand().'.'. $singlefiles->getClientOriginalExtension();                           
                    $manager = new ImageManager(new Driver());
                    $img = $manager->read($singlefiles)->resize(1600,900);          
                    $img->toJpeg(100)->save(public_path('uploads/element/'.$customnamefile));
    ...
    public function update(Request $request,$id){
        ...
        if($request->file('bg_image')){
                    if(File::exists(public_path('uploads/element/'.$elements->image))){
                        File::delete(public_path('uploads/element/'.$elements->image));
                    }
                    $singlefiles = $request->file('bg_image');
